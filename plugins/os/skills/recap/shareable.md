...
name: recap
description: >
  Use when the user says "/recap", "recap du jour", "recap d'aujourd'hui",
  "wrap-up semaine", "wrap-up mois", "wrap-up année", or similar.
  Generates activity recaps from GitHub, Linear, Claude Code conversations,
  Claude Code usage, and Notion.
---

# Skill `/recap`

Tu es un assistant qui génère des récaps d'activité. Tu agrèges 5 sources (GitHub, Linear, conversations Claude Code, ccusage, Notion) et produis des fichiers markdown dans le vault Obsidian.
...
- Jour → `recaps/{YYYY-MM}/{YYYY-MM-DD}.md` (fichier journalier) + mise à jour `semaine-{WW}.md` (ligne résumé)
- Semaine → `recaps/{YYYY-MM}/semaine-{WW}.md` (résumé) + mise à jour `mensuel.md`
- Mois → `recaps/{YYYY-MM}/mensuel.md` + mise à jour `recaps/{YYYY}.md`
- Année → `recaps/{YYYY}.md`
...
```bash
# Nombre de commits
gh api search/commits -f q='author:$GH_USERNAME committer-date:START_DATE..END_DATE' -f per_page=1 --jq '.total_count'

# PRs créées (avec détails)
gh api search/issues -f q='author:$GH_USERNAME type:pr created:START_DATE..END_DATE' -f per_page=100 --jq '.items | map({title, number: .number, html_url: .html_url, repo: (.repository_url | split("/") | last), state, created_at, merged_at: .pull_request.merged_at})'

# PRs mergées dans la période (créées avant, mergées dans la plage)
gh api search/issues -f q='author:$GH_USERNAME type:pr merged:START_DATE..END_DATE' -f per_page=100 --jq '.items | map({title, number: .number, html_url: .html_url, repo: (.repository_url | split("/") | last), merged_at: .pull_request.merged_at})'

# PRs reviewées (par d'autres auteurs)
gh api search/issues -f q='reviewed-by:$GH_USERNAME -author:$GH_USERNAME type:pr updated:START_DATE..END_DATE' -f per_page=100 --jq '.items | map({title, number: .number, html_url: .html_url, repo: (.repository_url | split("/") | last)})'
```
...
2. Pour chaque session, extraire :
    - Le nom du projet (du chemin : `-home-$USER-projects-my-project` → `my-project`)
...
Utilise `mcp__notion__notion-search` avec le filtre `created_by_user_ids` pour ne récupérer que les résultats liés à l'utilisateur.

- **Notion user ID** : `$NOTION_USER_ID` (`$USER_DISPLAY_NAME`)
- **OBLIGATOIRE** : toujours passer `filters: { created_by_user_ids: ["$NOTION_USER_ID"] }` pour éviter le bruit de pages non pertinentes
...
Template du fichier journalier :
...
1. Lire le fichier `semaine-{WW}.md` existant, ou créer le squelette suivant si inexistant :
...
2. **Tableau "Jours"** : ajouter/remplacer la ligne du jour (idempotent — matcher sur la date dans le lien). Format :
```
| [{Jour_FR} {DD}]({YYYY-MM-DD}.md) | {N} | {N} | {N} | ${cost} | {thème court} |
```

3. **Claude Code usage** : ajouter/mettre à jour la ligne du jour dans le tableau. Format :
```
...
Le fichier hebdomadaire **ne contient plus le détail des PRs par jour**, mais un résumé avec liens vers les fichiers journaliers.

Template du fichier hebdomadaire :
...
3. Réécrire `mensuel.md` complet avec le template :
...
4. Mettre à jour `recaps/{YYYY}.md` (créer si inexistant).
...
## Phase 4 : Écrire les fichiers

Pour `/recap jour` (aujourd'hui, hier, date ISO) :
1. `mkdir -p` le répertoire cible si nécessaire
2. Écrire `{YYYY-MM-DD}.md` (fichier journalier) avec l'outil `Write`
3. Mettre à jour `semaine-{WW}.md` (ajouter/remplacer la ligne du jour dans le tableau "Jours", mettre à jour les chiffres, Claude Code usage, et tickets Linear)

Pour `/recap semaine` :
1. Lire tous les `{YYYY-MM-DD}.md` de la semaine (les créer si manquants)
2. Réécrire `semaine-{WW}.md` complet (résumé avec liens vers fichiers journaliers)
3. Mettre à jour `mensuel.md`

Pour `/recap mois` :
1. Lire tous les `semaine-{WW}.md` du mois
2. Réécrire `mensuel.md` complet
3. Mettre à jour `recaps/{YYYY}.md`

Pour `/recap année` :
1. Lire tous les `mensuel.md` de l'année
2. Réécrire `recaps/{YYYY}.md`
...
- Suggestion de la prochaine action :
    - Après un récap jour → suggérer `/recap semaine` quand la semaine est terminée
    - Après un wrap-up semaine → suggérer `/recap mois` quand le mois est terminé
    - Après un wrap-up mois → suggérer `/recap année` quand l'année est terminée

---

## Règles importantes

1. **Idempotence** : relancer `/recap aujourd'hui` réécrit le fichier journalier et remplace la ligne du jour dans le résumé hebdo (pas de doublon)
